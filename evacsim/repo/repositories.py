...
class ScenarioRepository:
    ...
    def mark_simulation_executed(self, scenario_id: int) -> None:
        """
        指定した scenario_id の simulation_settings を
        実行済みに更新する
        """
...
class SimulationResultRepository:
    ...
    def insert_simulation_result(self, r: SimulationResult) -> int:
        with self.conn.cursor() as cur:
            cur.execute(
                """
            ...
            row = cur.fetchone()
            return row["result_id"] if isinstance(row, dict) else row[0]

    def insert_result_metric(self, m: ResultMetric) -> int:
        with self.conn.cursor() as cur:
            cur.execute(
                """
                ...
                (
                    ...
                    m.v2v_rate,
                    m.early_rate,
                    m.route_changed_vehicle_count,
                    m.normalcy_bias_count,
                    m.majority_bias_count,
                    m.shelter_congestion_count,
                    m.shelter_capacity_full_count,
                    m.is_deleted,
                ),
            )
            row = cur.fetchone()
    ...
    def insert_arrival_time_series(self, a: ArrivalTimeSeries) -> int:
        with self.conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO arrival_times_series (
                    result_id,
                    scenario_id,
                    veh_id,
                    arrival_time,
            ...
            row = cur.fetchone()
            return row["series_id"] if isinstance(row, dict) else row[0]

    def insert_arrival_time_series_bulk(
        self,
        *,
        result_id: int,
        scenario_id: int,
        arrival_time_by_vehID_dict: dict[str, float],
    ) -> None:
        if not arrival_time_by_vehID_dict:
            return
        ...
        values = [
            ...
            for veh_id, arrival_time in arrival_time_by_vehID_dict.items()
        ]
    ...
    def save_simulation_result_with_metrics(
        self,
        *,
        scenario_id: int,
        v2v_rate: float,
        early_rate: float,
        route_changed_vehicle_count: int | None,
        normalcy_bias_count: int | None,
        majority_bias_count: int | None,
        shelter_congestion_count: int | None,
        shelter_capacity_full_count: int | None,
        arrival_time_by_vehID_dict: dict[str, float] | None = None,
    ) -> tuple[int, int]:
        """
        simulation_results -> result_metrics -> arrival_times_series
        をまとめて保存する。
        戻り値: (result_id, metric_id)
        """
        result_id = self.insert_simulation_result(
            SimulationResult(
                scenario_id=scenario_id,
            )
        )

        metric_id = self.insert_result_metric(
            ResultMetric(
                result_id=result_id,
                scenario_id=scenario_id,
                v2v_rate=v2v_rate,
                ...
                normalcy_bias_count=normalcy_bias_count,
                majority_bias_count=majority_bias_count,
                shelter_congestion_count=shelter_congestion_count,
                shelter_capacity_full_count=shelter_capacity_full_count,
            )
        )

        if arrival_time_by_vehID_dict:
            self.insert_arrival_time_series_bulk(
                result_id=result_id,
                scenario_id=scenario_id,
                arrival_time_by_vehID_dict=arrival_time_by_vehID_dict,
            )

        return result_id, metric_id