...
from .repo.repositories import ScenarioRepository
...
def save_scenario_config(scenario_name: str, scenario_id: int):
    current_script = Path(__file__).resolve()
    evacsim_dir = current_script.parent
    root_dir = evacsim_dir.parent  # /Users/kashiisamutakeshi/vehicle-assistant-system
    
    # 正しいディレクトリ構造を定義
    config_dir = root_dir / "scenarios" / scenario_name / "configs"
    config_dir.mkdir(parents=True, exist_ok=True)
    
    # 絶対パスでのファイルパス
    absolute_file_path = config_dir / f"config_scenario_{scenario_id}.toml"
    
    # 結果: scenario/pervehicle/pervehicle/configs/config_scenario_1.toml
    relative_file_path = absolute_file_path.relative_to(root_dir)
    
    return absolute_file_path, relative_file_path

if __name__ == "__main__":
    # its102は対応できていない
    # python3 -m evacsim.run_batch_simulations pervehicle  1 2 
    ...
    try:
        ...
        scenario_id_from = sys.argv[2]    # 例: "1"
        scenario_id_to = sys.argv[3]      # 例: "10"
        ...
        for scenario_id in range(int(scenario_id_from), int(scenario_id_to) + 1):
            db = Database.from_env()
            with db.transaction() as conn:
                repo = ScenarioRepository(conn)
                simulation_params = repo.get_scenario_all_params(scenario_id=scenario_id)
            print(f"Loaded scenario_id={scenario_id} from DB.")
            toml_string = toml.dumps(simulation_params)
            print(f"Params: {toml_string}")
            NUM_RUNS = simulation_params['num_simulations']
            NUM_RUNS = 1 # デバッグ用に1回だけ実行
            abs_path, config_path = save_scenario_config(scenario_name=scenario_name, scenario_id=scenario_id)

            with open(abs_path, "w", encoding="utf-8") as f:
                f.write(toml_string)
            print(f"=== [Scenario={scenario_id}] 全ジョブを並列実行（workers={MAX_WORKERS}） ===")
