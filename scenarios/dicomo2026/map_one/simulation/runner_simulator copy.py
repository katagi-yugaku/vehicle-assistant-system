...
def control_vehicles():
    ...
    for current_vehID in vehIDs:
        ...
        if not vehInfo_by_current_vehID.get_arrival_flag():
            # AGENT: 乗り捨てが現在のpositionで成功しなかった場合、次の場所で乗り捨てを試みる
            ...
            if is_vehID_in_congested_edge(vehID=current_vehID, threshold_speed=THRESHOLD_SPEED) and is_step_5:
                ...
                # AGENT: 経路を変更をとるか否か
                current_motivation_for_route_change = calculate_motivation_for_evacuation_action(
                                                                                agent=agent_by_current_vehID,
                                                                                current_time=current_time,
                                                                                action="rc",
                                                                                debug=False,
                                                                                )
                if current_motivation_for_route_change >= agent_by_current_vehID.get_route_change_threshold():
                    current_routeID = traci.vehicle.getRouteID(current_vehID)
                    # 経路を変更する
                ...
                # AGENT: 逆走行為を取るのか否か
                current_motivation_for_wrong_way_driving = calculate_motivation_for_evacuation_action(
                                                                                                        agent=agent_by_current_vehID,
                                                                                                        current_time=current_time,
                                                                                                        action="ww",
                                                                                                        debug=False,
                                                                                                        )
                if current_motivation_for_wrong_way_driving >= agent_by_current_vehID.get_vehicle_abandoned_threshold():
                    print("逆走行為を行います current_vehID: {}, current_edgeID: {}, time: {}, motivation: {}, thresshold: {}".format(current_vehID, current_edgeID, current_time, current_motivation_for_wrong_way_driving, agent_by_current_vehID.get_vehicle_abandoned_threshold()))
                    traci.vehicle.setColor(current_vehID, (0, 255, 255))
                    traci.vehicle.changeLane(vehID=current_vehID, laneIndex=2, duration=1000)


                # AGENT: 車両乗り捨て行為をとるか否か
                if agent_by_current_vehID.get_shelter_full_info_obtained_flg() or agent_by_current_vehID.get_tsunami_info_obtained_flg():
            ...
            # INFO: 津波接近の前兆情報を取得した場合、車両情報を更新する
