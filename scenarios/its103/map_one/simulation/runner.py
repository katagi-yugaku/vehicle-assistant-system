...
def parse_args():
    p = argparse.ArgumentParser(description="runner parameters (except num_simulations)")
    # 既存で使っている3つ
    p.add_argument("--early_rate", type=float, required=True)
    p.add_argument("--vehicle_interval", type=float, required=True)
    p.add_argument("--insight_range", type=float, required=True)

    # CSV / TOML の見出し（num_simulations 以外）
    p.add_argument("--comm_range", type=float, required=True)
    p.add_argument("--num_vehicles", type=int, required=True)
    p.add_argument("--active_route_change_mean", type=float, required=True)
    p.add_argument("--active_route_change_var", type=float, required=True)
    p.add_argument("--cautious_route_change_mean", type=float, required=True)
    p.add_argument("--cautious_route_change_var", type=float, required=True)
    p.add_argument("--shelter_capacity_threshold", type=float, required=True)
    p.add_argument("--tsunami_sign_start_time", type=float, required=True)
    p.add_argument("--tsunami_sign_end_time", type=float, required=True)
    p.add_argument("--motivation_threshold_start", type=float, required=True)
    p.add_argument("--motivation_threshold_end", type=float, required=True)
    p.add_argument("--min_motivation_start", type=float, required=True)
    p.add_argument("--min_motivation_end", type=float, required=True)
    p.add_argument("--positive_lanechange_start", type=float, required=True)
    p.add_argument("--positive_lanechange_end", type=float, required=True)
    p.add_argument("--negative_lanechange_start", type=float, required=True)
    p.add_argument("--negative_lanechange_end", type=float, required=True)
    p.add_argument("--driver_visibility_distance", type=float, required=True)
    p.add_argument("--positive_majority_bias", type=float, required=True)
    p.add_argument("--negative_majority_bias", type=float, required=True)
    return p.parse_args()
...
if __name__ == "__main__":
    args = parse_args()
    early_rate: float = args.early_rate
    vehicle_interval: float = args.vehicle_interval
    INSIGHT_RANGE: float = args.insight_range
    COMMUNICATION_RANGE = args.comm_range
    TOTAL_VEHNUM = args.num_vehicles
    ACTIVE_ROUTE_CHANGE_MEAN = args.active_route_change_mean
    ACTIVE_ROUTE_CHANGE_VAR = args.active_route_change_var
    CAUTIOUS_ROUTE_CHANGE_MEAN = args.cautious_route_change_mean
    CAUTIOUS_ROUTE_CHANGE_VAR = args.cautious_route_change_var
    SHELTER_CAPACITY_THRESHOLD = args.shelter_capacity_threshold
    TSUNAMI_SIGN_START_TIME = args.tsunami_sign_start_time
    TSUNAMI_SIGN_END_TIME = args.tsunami_sign_end_time
    MOTIVATION_THRESHOLD_START = args.motivation_threshold_start
    MOTIVATION_THRESHOLD_END = args.motivation_threshold_end
    MIN_MOTIVATION_START = args.min_motivation_start
    MIN_MOTIVATION_END = args.min_motivation_end
    POSITIVE_LANECHANGE_START = args.positive_lanechange_start
    POSITIVE_LANECHANGE_END = args.positive_lanechange_end
    NEGATIVE_LANECHANGE_START = args.negative_lanechange_start
    NEGATIVE_LANECHANGE_END = args.negative_lanechange_end
    DRIVER_VISIBILITY_DISTANCE = args.driver_visibility_distance
    POSITIVE_MAJORITY_BIAS = args.positive_majority_bias
    NEGATIVE_MAJORITY_BIAS = args.negative_majority_bias
