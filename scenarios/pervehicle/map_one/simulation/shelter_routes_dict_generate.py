...
def find_start_edges_after_approach_edges(start_edgeID:str, custome_edge_list:list, approach_edge_list: list) -> list:
    approach_edge_list.append(start_edgeID)
    custome_edge: CustomeEdge = utilities.get_custome_edge_by_edgeID(custome_edge_list=custome_edge_list, edgeID=start_edgeID)
    next_edge_list_with_junction = custome_edge.obtain_neighbour_incom_edgeIDs_with_junc_by_start_junc()
    next_edge_list = [edge_ID for edge_ID in next_edge_list_with_junction if not edge_ID.startswith(':')]
    ...
    if len(next_edge_list) == 1:
        next_edge = next_edge_list[0]
        return find_start_edges_after_approach_edges(next_edge, custome_edge_list, approach_edge_list)
    elif len(next_edge_list) >= 0:
        # 交差点に到着した場合、終了
        # print(f'approach edges found: {approach_edge_list}')
        custome_edge: CustomeEdge = utilities.get_custome_edge_by_edgeID(custome_edge_list=custome_edge_list, edgeID=approach_edge_list[-1])
        next_edge_list_with_junction = custome_edge.obtain_neighbour_incom_edgeIDs_with_junc_by_start_junc()
        next_edge_list = [edge_ID for edge_ID in next_edge_list_with_junction if not edge_ID.startswith(':')]
...
def find_edges_until_shelter(start_edgeID:str, custome_edge_list:list, edges_until_shelter_list: list) -> list:
    edges_until_shelter_list.append(start_edgeID)
    custome_edge: CustomeEdge = utilities.get_custome_edge_by_edgeID(custome_edge_list=custome_edge_list, edgeID=start_edgeID)
    next_edge_list_with_junction = custome_edge.obtain_neighbour_incom_edgeIDs_with_junc_by_start_junc()
    if not next_edge_list_with_junction:
        next_edge_list_with_junction = custome_edge.obtain_neighbour_outgo_edgeIDs_with_junc_by_start_junc()
    next_edge_list = [edge_ID for edge_ID in next_edge_list_with_junction if not edge_ID.startswith(':')]
    if len(next_edge_list) == 1:
        next_edge = next_edge_list[0]
        return find_edges_until_shelter(next_edge, custome_edge_list, edges_until_shelter_list)
    elif len(next_edge_list) == 0 or len(next_edge_list) >=2:
        # 避難地あるいは交差点に到着した場合、終了
        return edges_until_shelter_list
    else:
        return []
...
if __name__ == "__main__":
    ...
    shelterID_by_edgeID:dict = {v: k for k, v in shelter_edge_by_IDs.items()}
    ...
    for start_edgeID in start_edgeIDs:
        aprroach_edgeIDs = []
        next_edgeIDs = find_start_edges_after_approach_edges(start_edgeID=start_edgeID, custome_edge_list=custome_edge_list, approach_edge_list=aprroach_edgeIDs)
        count = 0
        for next_edgeID in next_edgeIDs:
            edges_until_shelter_list = []
            edges_within_shelter = find_edges_until_shelter(start_edgeID=next_edgeID, custome_edge_list=custome_edge_list, edges_until_shelter_list=edges_until_shelter_list)
            print(f'test: {edges_within_shelter}')
            try:
                shelterID = shelterID_by_edgeID[edges_within_shelter[-1]]
                routes_by_shelterID[shelterID] = edges_within_shelter
                opposite_route_edgeIDs = []
                for edge in edges_within_shelter:
                    opposite_edgeID = utilities.get_opposite_edgeID_by_edgeID(edgeID=edge)
                    opposite_route_edgeIDs.append(opposite_edgeID)
                routes_by_shelterID[f"{shelterID}_opposite"] = opposite_route_edgeIDs

            except KeyError:
                routes_by_shelterID[f"intermediate_{count}"] = edges_within_shelter
                count += 1
                opposite_route_edgeIDs = []
                for edge in edges_within_shelter:
                    opposite_edgeID = utilities.get_opposite_edgeID_by_edgeID(edgeID=edge)
                    opposite_route_edgeIDs.append(opposite_edgeID)
                routes_by_shelterID[f"intermediate_{count}_opposite"] = opposite_route_edgeIDs
                count += 1
                pass

    print(f'routes_by_shelterID: {routes_by_shelterID}')

    
    # print(f'approach_edgeIDs_by_start_edgeID: {approach_edgeIDs_by_start_edgeID}')
    utilities.export_start_end_edgeIDs_to_json(start_end_edgeIDs=routes_by_shelterID,
                                                file_path="/Users/kashiisamutakeshi/vehicle-assistant-system/scenarios/pervehicle/map_one/data/edgeIDs_within_jucntion_to_shelter_by_shelter.json")
