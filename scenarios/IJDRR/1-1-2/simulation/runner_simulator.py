...
# =========================
# Runtime paths / local SUMO
# =========================
# This file is expected at:
#   vehicle-assistant-system/scenarios/IJDRR/1-1-2/simulation/<this_file>.py
#
# Resolve every important path from __file__ so execution does not depend on
# the current working directory.
HERE = Path(__file__).resolve()
SCENARIO_DIR = HERE.parent.parent
PROJECT_ROOT = HERE.parents[4]

LOCAL_SUMO_HOME = PROJECT_ROOT / "sumo"
LOCAL_SUMO_BIN_DIR = LOCAL_SUMO_HOME / "bin"
LOCAL_SUMO_TOOLS_DIR = LOCAL_SUMO_HOME / "tools"

DATA_DIR = SCENARIO_DIR / "data"
SUMO_CFG = DATA_DIR / "ishinomaki_one_two_one.sumocfg"

# Force Python-side SUMO modules (traci / sumolib) to come from the same local
# SUMO source tree as the executable. This avoids mixing a Homebrew executable
# with a different Python package version.
os.environ["SUMO_HOME"] = str(LOCAL_SUMO_HOME)

if not LOCAL_SUMO_TOOLS_DIR.is_dir():
    raise FileNotFoundError(
        "Local SUMO tools directory was not found: "
        f"{LOCAL_SUMO_TOOLS_DIR}"
    )

local_sumo_tools = str(LOCAL_SUMO_TOOLS_DIR)
if local_sumo_tools not in sys.path:
    sys.path.insert(0, local_sumo_tools)

# =========================
# Third-party libraries
# =========================
# Import TraCI from the local SUMO source tree. A normal SUMO 1.20.0
# tools/traci/__init__.py re-exports the public API from traci.main.
# If the local __init__.py is incomplete or empty, repair the package
# namespace at runtime from traci.main so every later ``import traci``
# (including imports inside evacsim modules) sees the complete API.
import traci as _traci_package  # noqa: E402

if not hasattr(_traci_package, "start"):
    try:
        from traci import main as _traci_main  # noqa: E402
    except ImportError as exc:
        raise ImportError(
            "The local SUMO TraCI package is incomplete and "
            "traci.main could not be imported. Check "
            f"{LOCAL_SUMO_TOOLS_DIR / 'traci'}."
        ) from exc

    for _name, _value in vars(_traci_main).items():
        if not _name.startswith("_"):
            setattr(_traci_package, _name, _value)

traci = _traci_package

_REQUIRED_TRACI_ATTRIBUTES = (
    "start",
    "close",
    "simulationStep",
    "simulation",
...
_missing_traci_attributes = [
    name for name in _REQUIRED_TRACI_ATTRIBUTES
    if not hasattr(traci, name)
]
if _missing_traci_attributes:
    raise ImportError(
        "The local TraCI module is missing required attributes: "
        + ", ".join(_missing_traci_attributes)
        + f". Imported from: {getattr(traci, '__file__', '<unknown>')}"
    )
...
def resolve_local_sumo_binary(*, nogui: bool) -> str:
    """Return the validated local SUMO executable path.

    The ordinary command ``sumo`` may resolve to ``/opt/homebrew/bin/sumo``.
    This function intentionally bypasses PATH and selects the locally built
    executable under ``vehicle-assistant-system/sumo/bin``.
    """
    executable_name = "sumo" if nogui else "sumo-gui"
    binary = LOCAL_SUMO_BIN_DIR / executable_name

    if not binary.is_file():
        raise FileNotFoundError(
            f"Local SUMO executable was not found: {binary}"
        )
    if binary.stat().st_size == 0:
        raise RuntimeError(
            f"Local SUMO executable is empty (0 bytes): {binary}"
        )
    if not os.access(binary, os.X_OK):
        raise PermissionError(
            f"Local SUMO executable is not executable: {binary}"
        )
...
def validate_runtime_paths() -> None:
    """Fail early when the local SUMO tree or scenario files are missing."""
    required_paths = {
        "project root": PROJECT_ROOT,
        "local SUMO home": LOCAL_SUMO_HOME,
        "local SUMO bin": LOCAL_SUMO_BIN_DIR,
        "local SUMO tools": LOCAL_SUMO_TOOLS_DIR,
        "scenario data directory": DATA_DIR,
        "SUMO configuration": SUMO_CFG,
    }
...
def control_vehicles():
    ...
    for current_vehID in vehIDs:
        ...
        # 現行実装では渋滞判定を無効化中。以下は意図確認が必要な保留コード。
...
if __name__ == "__main__":
    # Usage:
    # python3 -m scenarios.IJDRR.1-1-2.simulation.runner_simulator \
    #   scenarios/JIP/configs/config_scenario_1.toml \
    #   1.0 \
    #   1.0 \
    #   --nogui
    parser = argparse.ArgumentParser(
        description="Run the evacuation simulation with a TOML configuration.",
    )
    parser.add_argument(
        "config",
        type=Path,
    ...
    parser.add_argument(
        "early_rate",
        type=float,
        help="Rate of early-decision agents.",
    )
    parser.add_argument(
        "v2v_capable_vehicle_rate",
        type=float,
        help="Rate of vehicles equipped with V2V communication.",
    )
    parser.add_argument(
        "--nogui",
        action="store_true",
        help="Run the command-line SUMO binary instead of sumo-gui.",
    )
    options = parser.parse_args()

    toml_path = options.config.expanduser().resolve()
    early_rate: float = options.early_rate
    v2v_capable_vehicle_rate: float = options.v2v_capable_vehicle_rate

    if not 0.0 <= early_rate <= 1.0:
        parser.error(f"early_rate must be between 0 and 1: {early_rate}")
    if not 0.0 <= v2v_capable_vehicle_rate <= 1.0:
        parser.error(
            "v2v_capable_vehicle_rate must be between 0 and 1: "
            f"{v2v_capable_vehicle_rate}"
        )

    # Validate paths before loading the simulation and force use of the
    # locally built SUMO executable rather than /opt/homebrew/bin/sumo.
    validate_runtime_paths()
    sumoBinary = resolve_local_sumo_binary(nogui=options.nogui)

    if not toml_path.is_file():
        parser.error(f"TOML configuration file was not found: {toml_path}")
    cfg = load_toml(toml_path)

    print(f"[SUMO] SUMO_HOME={LOCAL_SUMO_HOME}")
    print(f"[SUMO] binary={sumoBinary}")
    print(f"[SUMO] config={SUMO_CFG}")
    print(f"[SUMO] traci={Path(traci.__file__).resolve()}")

    # random と NumPy の乱数生成器を、それぞれシステム由来の状態で再初期化する。
    # 呼び出し回数と順序を維持するため、この2行は統合・削除しない。
    ...
    # 旧方式: route_prob_list = utilities.generate_route_prob_list(max_route_num)
    ...
    # 経路変更イベントの詳細は route_change_events に保持し、通常は出力しない。
